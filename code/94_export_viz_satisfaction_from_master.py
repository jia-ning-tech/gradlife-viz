...
- 基于 master_person_wide + satisfaction_long，
  生成“各方面满意度 × 高压组”的汇总表，供前端可视化使用。

一行 = 一个 (满意度维度 × 高压组) 组合。

输出：
1) 分析用汇总表（更完整字段）：
   /workspace/output/11_satisfaction_master/satisfaction_by_stress.csv

2) 可视化用简化表：
   /workspace/output/08_viz_data/viz_satisfaction_by_stress.csv

字段说明（可视化表）：
- aspect_code        : v074_num ~ v087_num
- q_no               : Q27.a ~ Q27.n
- aspect_short       : 一个简短的维度名（从 question_text 中截取）
- aspect_text        : 完整题干（原样保留）
...
def make_aspect_short(text: str) -> str:
    """
    从长题干中提炼一个短标签。
    目前用一个很简单的规则：取最后一句最后的核心短语，
    比如 'Work-life balance' / 'Social environment' 等。
    """
    ...
    t = text.strip()
    # 很多 question_text 形如：
    # "How satisfied are you with ... ? Work-life balance [numeric]"
    # 先按 '?' 或 ':' 分割，再取最后一段
    for sep in ["?", ":", "â€”", "—"]:
        if sep in t:
...
def main():
    ...
    # === 1. 按 (aspect × 高压组) 汇总：总体 ===
    grp = df.groupby(["aspect_code", "q_no", "aspect_text", "aspect_short", "high_stress_group", "high_stress_label"])
    ...
    # 好看一点的排序：先按 aspect_order，再按 high_stress_group
    agg = agg.sort_values(["aspect_order", "high_stress_group"])

    print("\n=== satisfaction_by_stress 总体预览（前 14 行）===")
    print(
        agg[[
            "aspect_code",
            "q_no",
    ...
    # === 2. 输出分析用 & 可视化用表 ===
    out_analysis = OUT_ANALYSIS_DIR / "satisfaction_by_stress.csv"
    out_viz = OUT_VIZ_DIR / "viz_satisfaction_by_stress.csv"

    agg.to_csv(out_analysis, index=False)
    # 可视化表用相同字段即可，前端可以直接用
    agg.to_csv(out_viz, index=False)

    print(f"\n已保存分析用满意度 × 高压汇总表到: {out_analysis}")
    print(f"已保存可视化用满意度 × 高压汇总表到: {out_viz}")
